...
# These rates have to be recalculated; probably just go back and rebuild this in the main file?

# create the augmented data set
crimeRatesMonthTypeVnV <- crimeRatesMonthType |> 
  mutate(
    crime.type = case_when(
      crime.type %in% c("Assault", "Homicide", "Rape", "Robbery") ~ "violent",
      crime.type %in% c("Arson", "Burglary", "Drugs", "MV Theft", "Theft") ~ "non-violent",
      TRUE ~ crime.type  # keep original if it doesn't match
    )
